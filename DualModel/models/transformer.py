...
class Transformer(CaptioningModel):
    def __init__(self, bos_idx, encoder, decoder):
        ...
        # self.register_state('features', None)
        # self.register_state('semantic_features', None)
        ...
        if mode == 'teacher_forcing':
            ...
            if t == 0:
                if not isinstance(visual, tuple) and not isinstance(visual, list):
                    visual = [visual]
                # self.grid_features, self.object_features = self.encoder(*visual)
                # self.mask_enc = None
                ...
                # self.features, self.mask_enc = self.encoder(grid_features, region_features)
                # self.grid_features, self.object_features, self.semantic_features, self.mask_enc = self.encoder(*visual)

                # self.decoder.init_prefix(self.grid_features, self.object_features, self.semantic_features, self.mask_enc)
                it = visual[0].data.new_full((visual[0].shape[0], 1), self.bos_idx).long() # (b_s,1)
            else:
                it = prev_output

        # return self.decoder.step(it, self.grid_features, self.object_features, self.mask_enc)
        return self.decoder(it, self.grid_features, self.object_features, self.mask_enc)
        # return self.decoder(it, self.features, self.mask_enc)
        # return self.decoder(it, self.grid_features, self.object_features, self.semantic_features, self.mask_enc)
...
class TransformerEnsemble(CaptioningModel):
    def __init__(self, model: Transformer, weight_files, device):
        ...
        for i in range(self.n):
            state_dict_i = torch.load(weight_files[i], map_location=device)['state_dict_o']
