...
from Camel.models.captioner import Captioner, CaptionerEnsemble
...
def test():
    ...
    # encoder = build_encoder(3, padding_idx=0, d_in=2560)
    # decoder = build_decoder(len(text_field.vocab), 54, 3, text_field.vocab.stoi['<pad>'])
    ...
    args.N_dec = 3
    args.d_in = 2560
    args.enable_mesh = False
    model = Captioner(args, text_field).to(args.device)
    ...
    model = CaptionerEnsemble(model, weight_files).to(args.device)
