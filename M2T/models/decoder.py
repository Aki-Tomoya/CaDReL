...
from common.models.transformer.attention import MultiHeadAttention, ScaledDotProductAttentionMemory
...
class MeshedDecoderLayer(Module):
    ...
    def forward(self, input, enc_dif_output, mask_pad, enc_output, mask_self_att, mask_enc_att):
...
class MeshedDecoder(Module):
    def __init__(self, vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1,return_logits = False,
                 self_att_module=None, enc_att_module=None, self_att_module_kwargs=None, enc_att_module_kwargs=None, batch_size=None):
    ...
    def forward(self, input, encoder_dif_output, mask_encoder, encoder_output):
        # input (b_s, seq_len)
        ...
        for i, l in enumerate(self.layers):
            out = l(out, encoder_dif_output, mask_queries, encoder_output, mask_self_attention, mask_encoder)
...
def build_decoder(vocab_size, max_len, N_dec, padding_idx, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1):
    return MeshedDecoder(vocab_size, max_len, N_dec, padding_idx, d_model, d_k, d_v, h, d_ff, dropout)

# def build_decoder(vocab_size, m, max_len, N_dec, padding_idx,  
#                   d_model=512, d_k=64, d_v=64, h=8, d_ff=2048, dropout=.1):
#     return TransformerDecoder(vocab_size, max_len, N_dec, padding_idx,  
#                                d_model, d_k, d_v, h, d_ff, dropout, 
#                                self_att_module=ScaledDotProductAttentionMemory, self_att_module_kwargs={'m': m},
#                                enc_att_module=ScaledDotProductAttentionMemory, enc_att_module_kwargs={'m': m})